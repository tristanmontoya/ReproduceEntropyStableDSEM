struct EulerDriver{ApproximationType <: AbstractApproximationType,
...
function EulerDriver(p; l=p, p_quad=35, C_t=0.01,
    ...
    M0 = 2, L=1.0, mach_number=0.4, γ = 1.4, n_periods=1, 
    mesh_perturb = 1/16, n_grids = 6, 
    load_from_file=true, overwrite=false, test_type=1)

    path = string(path, scheme, "_", element_type, "_",
        form, "_p", p, "/", numerical_flux, "/")
    ...
    return EulerDriver(l, p_quad, 
        C_t, scheme, element_type, form, strategy,
        ode_algorithm, path, M0, L, mach_number, γ, n_periods, mesh_perturb,
        n_grids, load_from_file, overwrite, test_type)
end

function run_driver(driver::EulerDriver)

    (; l,p_quad,C_t,scheme,element_type,form,strategy,ode_algorithm,path,
        M0,L,mach_number,γ, n_periods, mesh_perturb, n_grids, load_from_file, overwrite, test_type) = driver
    ...
    if (d == 2) && (test_type == 1)
        initial_data = IsentropicVortex(conservation_law, θ=0.0,Ma=mach_number,
        β=sqrt(2/(γ-1)*(1-0.75^(γ-1))), R=0.1, x_0=(L/2,L/2))
    ...
    for n in n_start:n_grids
        ...
        sol = solve(ode_problem, ode_algorithm, adaptive=false,
            ...
            results_path, (t0,T), ceil(Int, T/(dt*100)), restart_step))
