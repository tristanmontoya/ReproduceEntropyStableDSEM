struct EulerPRefinementDriver{
    ElemShape <: AbstractElemShape, ResidualForm <: AbstractResidualForm,
    Strategy <: AbstractStrategy, Algorithm <: OrdinaryDiffEqAlgorithm}
    p_min::Int
    p_max::Int
    p_quad::Int
    C_t::Float64
...
function EulerPRefinementDriver(p_min::Int, p_max::Int;
    p_quad=35, C_t=0.1, n_s=500,
    ...
    M0 = 16, L=1.0, mach_number=0.4, base_density=0.5, γ = 1.4, T=15.0, 
    mesh_perturb = 1/16, load_from_file=true, overwrite=false, test_type=1)
    ...
    form = eval(Symbol(form))(
        inviscid_numerical_flux=eval(Symbol(numerical_flux))())
    ode_algorithm = eval(Symbol(ode_algorithm))()
    strategy = eval(Symbol(strategy))()

    return EulerPRefinementDriver(p_min, p_max, p_quad, C_t, n_s, scheme,
        element_type, form, strategy, ode_algorithm, path, M0, L, mach_number, base_density, γ, T, mesh_perturb, load_from_file, overwrite, test_type)
end

function run_driver(driver::EulerPRefinementDriver)

    (; p_min, p_max, p_quad, C_t,n_s,scheme,element_type,form,strategy,
        ode_algorithm, path, M0,L,mach_number, base_density, γ, T, mesh_perturb,
        load_from_file, overwrite, test_type) = driver
    ...
    elseif test_type == 2 # robustness
        ...
        elseif d == 2
            L = 2.0
            initial_data = KelvinHelmholtzInstability(conservation_law,
                base_density)
            a = 0.5
        elseif d == 3
            L = 2π
    ...
    for p in p_start:p_max
        ...
        reset_timer!()
        integrator = init(ode_problem, ode_algorithm, adaptive=false,
        ...
        if test_type == 1
            error_analysis = ErrorAnalysis(results_path, conservation_law, 
                spatial_discretization, DefaultQuadrature(p_quad))
                
            error = analyze(error_analysis, last(integrator.sol.u),
                initial_data)
        ...
        save_object(string(path, "end_times.jld2"), 
            push!(end_times, integrator.t))
        ...
        entropy = analyze(EntropyConservationAnalysis(results_path, 
            conservation_law, spatial_discretization), time_steps)
        save_object(string(results_path, "conservation.jld2"),
            conservation.E)
        save_object(string(results_path, "conservation_residual.jld2"),
            conservation.dEdt)
        save_object(string(results_path, "entropy.jld2"),entropy.E)
        save_object(string(results_path, "entropy_residual.jld2"),
            entropy.dEdt)
    end
end